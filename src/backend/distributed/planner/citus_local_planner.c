...
ErrorIfUnsupportedCitusLocalQuery(Query *query, RTEListProperties *rteListProperties)
{
	...
	bool queryIsNotSimpleSelect = FindNodeCheck((Node *) query, QueryIsNotSimpleSelect);

	if (queryIsNotSimpleSelect && rteListProperties->hasReferenceTable)
	{
		ereport(ERROR, (errmsg("cannot plan SELECT FOR UPDATE queries with local tables "
